...
public class Logging {
    ...
    public static <InputType, OutputType> OutputType withLogging(String messagePrefix, InputType input, Action<InputType, OutputType> action) {
        String stringRepresentationOfRequest = ToStringBuilder.reflectionToString(input);

        logger.info(messagePrefix + " - Got: " + stringRepresentationOfRequest);
        try {

            OutputType response = action.call(input);
